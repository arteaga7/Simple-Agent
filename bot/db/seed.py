"""Catalog seeding helpers (replaces the old precios_proveedor dict).
...
SEED_PRODUCTS = [
    {"name": "camisa", "price": 100, "stock_quantity": 50, "description": "Camisa de vestir"},
    {"name": "pantalón", "price": 250, "stock_quantity": 30, "description": "Pantalón de vestir"},
    {"name": "corbata", "price": 75, "stock_quantity": 80, "description": "Corbata clásica"},
]
...
def seed_products(db: Session, products: list[dict] | None = None) -> int:
    ...
    existing = {name.lower() for name in db.scalars(select(Product.name)).all()}
    created = 0
